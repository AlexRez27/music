...
import { songsCollection, storage, commentsCollection } from '@/includes/firebase';
...
export default {
  ...
  emits: ['update-song', 'remove-song', 'update-unsaved-flag'],
  data() {
    return {
      showForm: false,
      schema: {
  ...
  methods: {
    ...
    async deleteSong() {
      ...
      const snapshots = await commentsCollection.where('sid', '==', this.song.docID)
        .get();

      await snapshots.forEach((document) => {
        commentsCollection.doc(document.id).delete();
      });

      this.$emit('remove-song', this.index);
    },
    // remove(i) {
